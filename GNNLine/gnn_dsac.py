...
class GNNDSAC:
	...
	def __call__(self, prediction, log_probs, labels):
		'''
		...
		self.g_log_probs = torch.zeros(batch_size, self.hyps, prediction.size(2)) # gradient tensor for neural guidance

		for b in range(0, batch_size):

			hyp_losses = torch.zeros(self.hyps) # loss of each hypothesis
			hyp_scores = torch.zeros(self.hyps) # score of each hypothesis
			_hyp_scores = torch.zeros(self.hyps)
			...
			for h in range(0, self.hyps):	
				...
				slope, intercept, valid = self.__sample_hyp(x, y, p, self.g_log_probs[b][h])
				if not valid: 
					hyp_losses[h] = 100
					hyp_scores[h] = 0.0001
		...
		return avg_exp_loss / batch_size, _hyp_scores