...
# prepare GNN input using KNN
def prepare_gnn_input(feat):
	_feat = (feat.detach().numpy()[0].reshape(256, -1)).T
	A = kneighbors_graph(_feat, 4, mode='connectivity', include_self=False)
	edge_index, edge_weights = from_scipy_sparse_matrix(A)
	gnn_input = torch.tensor(_feat, dtype=torch.double)
	return gnn_input.float(), edge_index, edge_weights.float()
...
class LineNN(nn.Module):
	...
	def __init__(self, net_capacity, receptive_field = 0, direct = False, image_size = 64, global_output_grid = 8):
		...
		self.graphconv1 = GraphConv(64*c, 64)
		self.graphbn1 = BatchNorm(64)
		self.graphconv2 = GraphConv(64, 32)
		self.graphbn2 = BatchNorm(32)
		self.graphconv3 = GraphConv(32, 1)
	...
	def forward(self, input):
		...
		a = x.detach()
		gnn_input, edge_index, edge_weights = prepare_gnn_input(a)
		x = self.fc3(x)
		...
		# second branch
		x2 = F.relu(self.graphbn1(self.graphconv1(gnn_input, edge_index, edge_weights)))
		x2 = F.relu(self.graphbn2(self.graphconv2(x2, edge_index, edge_weights)))
		log_probs = self.graphconv3(x2, edge_index, edge_weights)
		log_probs = log_probs.view(batch_size, -1)

		# normalize output to 0,1
		log_probs = F.logsigmoid(log_probs) 
		...
		return x.view(batch_size, 2, -1), norm_log_probs